...
### Simulated networks
PAT-like networks should exhibit properties of small-world network growth, namely a low mean path length and a high clustering coefficient [@watts_collective_1998; @amaral_classes_2011; @steyvers_large-scale_2005]: words should be more densely connected, with shorter connections between words, especially in earlier development. To test this, mean path length and clustering coefficient values were generated for both the Target and Actual networks, as outlined below. Data were compared to the growth of a simulated small-world network, known as a Watts-Strogatz network [@watts_collective_1998]; if no statistical differences are observed between the real and simulated data over the trajectory of development, then a PAT-like model of network growth may be present in the data. The real data were also compared to a similarly-sized but randomly-generated network known as a Erdős–Rényi model. If the real network grows in a systematic way, then we would expect the real data to differ significantly from the randomly-generated Erdős–Rényi network. To run these analyses, network graphs were computed for each child at each month in the dataset, such that the network grew cumulatively over time - all words produced in the given month and all previous months were included in each month's network. Mean path length and clustering coefficient were calculated for each graph, and then compared to the two kinds of simulated data, matched for network size: a Watts-Strogatz network (prototypical PAT-like network) and an Erdős–Rényi network (random network). Watts-Strogatz simulations match both the mean degree and the network size of the network in each month; Erdős–Rényi networks require only network size to be specified.
...
To test RQ2, the phonological distance between target and actual forms was taken as a proxy of word selection and adaptation. That is, if a word is produced in a target-like way (i.e. assumed to be selected[^1]), then the phonological distance between the Target form and the way it is produced (Actual) should be low. The opposite is likely true for adapted forms, as we expect, by definition, a non-target-like production and thus a higher distance between Target and Actual form. Following Vihman's [REF] framework, we would thus expect low distance been Actual and Target forms earlier on in development, and higher distance later. To test this, I drew on generalized additive models (GAMs) and generalized additive mixed effects models (GAMMs), since these allow the analysis of non-linear change over time, and can account for statistical differences between two non-linear trajectories of data that may differ in non-linear ways [REFs].
...
```{r network graph model comparisons - actual, message=FALSE, warning=FALSE, include=FALSE}

# Mean path length

global_pathlength_A <- lm(path_length ~ age + corpus + numNodes,
                                             data=subset(SWD_red,
                                                           data_type == "actual"))
output.globalpathlength_A <- summary(global_pathlength_A)

global_pathlength_T <- lm(path_length ~ age + corpus + numNodes,
                                             data=subset(SWD_red,
                                                           data_type == "target"))
output.globalpathlength_T <- summary(global_pathlength_T)
...
```{r network graph model comparisons - target, message=FALSE, warning=FALSE, include=FALSE}

# Mean path length

global_pathlength_T <- lmerTest::lmer(path_length ~ age + corpus + numNodes + (1+age|Speaker),
                                             data=subset(SWD_red,
                                                           data_type == "target")
                                                           ,
                                       REML=FALSE)
#summary(global_pathlength_T)

global_pathlength_T_null <- lmerTest::lmer(path_length ~ corpus + numNodes + (1+age|Speaker),
                                             data=subset(SWD_red,
                                                           data_type == "target")
                                                           ,
                                       REML=FALSE)

pathlength_T <- anova(global_pathlength_T, global_pathlength_T_null) # not significant
...
```{r small world properties - path length, message=FALSE, warning=FALSE, include=FALSE}
...
# wilcox.test(x = subset(SWD_red, corpus == "English"  & data_type == "actual")$path_length,
#             subset(SWD_red, corpus == "English"  & data_type == "WS_actual")$path_length, 
#             conf.int = T)
# 
# wilcox.test(x = subset(SWD_red, corpus == "English"  & data_type == "actual")$path_length,
#             subset(SWD_red, corpus == "English"  & data_type == "Erdos_Renyi")$path_length, 
#             conf.int = T)

# Target

global_pathlength_model_T_P <- lmerTest::lmer(path_length ~ data_type * age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "English" &
...
global_pathlength_model_T_P_null <- lmerTest::lmer(path_length ~ age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "English"  &
                                                             data_type %in% c("target", "WS_target", "Erdos_Renyi")),
                                       REML=FALSE)

pathlength_WS_T_P <- anova(global_pathlength_model_T_P, global_pathlength_model_T_P_null)

pathlength_WS_T_P_df <- pathlength_WS_T_P$Df[2]
pathlength_WS_T_P_chisq <- pathlength_WS_T_P$Chisq[2]
pathlength_WS_T_P_p.value <- pathlength_WS_T_P$`Pr(>Chisq)`[2]

# wilcox.test(x = subset(SWD_red, corpus == "English"  & data_type == "target")$path_length,
#             subset(SWD_red, corpus == "English"  & data_type == "WS_target")$path_length, 
#             conf.int = T)
# 
# wilcox.test(x = subset(SWD_red, corpus == "English"  & data_type == "target")$path_length,
#             subset(SWD_red, corpus == "English"  & data_type == "Erdos_Renyi")$path_length, 
...
# wilcox.test(x = subset(SWD_red, corpus == "French"  & data_type == "actual")$path_length,
#             subset(SWD_red, corpus == "French"  & data_type == "WS_actual")$path_length, 
#             conf.int = T)
# 
# wilcox.test(x = subset(SWD_red, corpus == "French"  & data_type == "actual")$path_length,
#             subset(SWD_red, corpus == "French"  & data_type == "Erdos_Renyi")$path_length, 
#             conf.int = T)

# Target

global_pathlength_model_T_L <- lmer(path_length ~ data_type * age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "French" &
                                                             data_type %in% c("target", "WS_target", "Erdos_Renyi")),
                                       REML=FALSE)
...
global_pathlength_model_T_L_null <- lmer(path_length ~ age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "French" &
                                                             data_type %in% c("target", "WS_target", "Erdos_Renyi")),
                                       REML=FALSE)

pathlength_WS_T_L <- anova(global_pathlength_model_T_L, global_pathlength_model_T_L_null)

pathlength_WS_T_L_df <- pathlength_WS_T_L$Df[2]
pathlength_WS_T_L_chisq <- pathlength_WS_T_L$Chisq[2]
pathlength_WS_T_L_p.value <- pathlength_WS_T_L$`Pr(>Chisq)`[2]


# wilcox.test(x = subset(SWD_red, corpus == "French"  & data_type == "target")$path_length,
#             subset(SWD_red, corpus == "French"  & data_type == "WS_target")$path_length, 
#             conf.int = T)
# 
# wilcox.test(x = subset(SWD_red, corpus == "French"  & data_type == "target")$path_length,
#             subset(SWD_red, corpus == "French"  & data_type == "Erdos_Renyi")$path_length, 
...
```{r Mean path length model outputs, include=FALSE}
order <- c("Intercept", "Age", "Watts-Strogatz comparison", "Erdős–Rényi comparison")

table.MPL.model.output <- rbind(pathlength_WS_A_P, pathlength_WS_T_P, pathlength_WS_A_L, pathlength_WS_T_L) %>%
  rownames_to_column(var="Model") %>%
  filter(Chisq > 0) %>%
  rename(#"PA Df"="Df",
         #"PA Chisq"="Chisq",
         "p"=`Pr(>Chisq)`) %>%
  mutate(Corpus = ifelse(Model == "global_pathlength_model_A_P", "English", " "),
         Corpus = ifelse(Model == "global_pathlength_model_A_L", "French", Corpus),
         Model=fct_recode(Model,
                           "Actual"="global_pathlength_model_A_P",
                            "Target"="global_pathlength_model_T_P",
                            "Actual"="global_pathlength_model_A_L",
                            "Target"="global_pathlength_model_T_L"),
             p=scales::pvalue(p)) %>%
  rename(`Data type` = "Model") %>%
  select(Corpus, `Data type`, `Df`, `Chisq`, `p`)

model.summary.P_A_pathlength <- summary(global_pathlength_model_A_P)

pathlength_P_A_tab <- model.summary.P_A_pathlength$coefficients %>%
  as.data.frame %>%
  rename(
    "beta"="Estimate"
     , "SE"="Std. Error"
  ...
  mutate(beta=as.numeric(beta),
        ...
        `p`=as.numeric(`p`),
             p=scales::pvalue(p)) %>%
  mutate(Corpus = ifelse(Effect == "Intercept", "English", ""),
        # `Data type` = ifelse(Effect == "Intercept", "Actual", ""),
         Effect = fct_recode(Effect,
                             "Watts-Strogatz comparison" = "Data typeWS actual",
                             "Erdős–Rényi comparison" = "Data typeErdos Renyi")) %>%
filter(Effect %in% c("Intercept", "Age", "Watts-Strogatz comparison", "Erdős–Rényi comparison")) %>%
  slice(match(order, Effect)) %>%
  select(Corpus, Effect, beta, SE, t, p)

model.summary.P_T_pathlength <- summary(global_pathlength_model_T_P)

pathlength_P_T_tab <- model.summary.P_T_pathlength$coefficients %>%
  as.data.frame %>%
  rename(
    "beta"="Estimate"
     , "SE"="Std. Error"
  ...
  mutate(beta=as.numeric(beta),
        ...
        `p`=as.numeric(`p`),
             p=scales::pvalue(p)) %>%
  mutate(#Corpus = ifelse(Effect == "Intercept", "English", ""),
        # `Data type` = ifelse(Effect == "Intercept", "Target", ""),
         Effect = fct_recode(Effect,
                             "Watts-Strogatz comparison" = "Data typeWS target",
                             "Erdős–Rényi comparison" = "Data typetarget")) %>%
filter(Effect %in% c("Intercept", "Age", "Watts-Strogatz comparison", "Erdős–Rényi comparison")) %>%
  slice(match(order, Effect)) %>%
  dplyr::select(-Effect)

model.summary.L_A_pathlength <- summary(global_pathlength_model_A_L)

pathlength_L_A_tab <- model.summary.L_A_pathlength$coefficients %>%
  as.data.frame %>%
  rename(
    "beta"="Estimate"
     , "SE"="Std. Error"
  ...
  mutate(beta=as.numeric(beta),
        ...
        `p`=as.numeric(`p`),
             p=scales::pvalue(p)) %>%
  mutate(Corpus = ifelse(Effect == "Intercept", "French", ""),
       #  `Data type` = ifelse(Effect == "Intercept", "Actual", ""),
         Effect = fct_recode(Effect,
                             "Watts-Strogatz comparison" = "Data typeWS actual",
                             "Erdős–Rényi comparison" = "Data typeErdos Renyi")) %>%
...
model.summary.L_T_pathlength <- summary(global_pathlength_model_T_L)

pathlength_L_T_tab <- model.summary.L_T_pathlength$coefficients %>%
  as.data.frame %>%
  rename(
    "beta"="Estimate"
     , "SE"="Std. Error"
...
pathlength_english <- cbind(pathlength_P_A_tab, pathlength_P_T_tab)
pathlength_french <- cbind(pathlength_L_A_tab, pathlength_L_T_tab)

pathlength.model.outputs <- rbind(pathlength_english, pathlength_french)
rownames(pathlength.model.outputs) <- NULL

```

```{r table-pathlength-modeloutput, echo=F, message=FALSE, warning=FALSE, comment=F, results="asis"}
cap="Outputs from nested model comparisons testing the effect of data type (Real vs. Watts-Strogatz vs. Erdős–Rényi) on path length. English and French data, both Actual and Target."
knitr::kable(table.MPL.model.output, "latex", booktabs=T, longtable=T, caption=cap, digits=2, align="c") %>%
  kable_styling()%>%
  row_spec(9, hline_after=T)

```

```{r table-pathlength, echo=F, message=FALSE, warning=FALSE, comment=F, results="asis"}
cap="Outputs from linear regression models testing comparisons of Real vs. Simulated data on path length. English and French data, both Actual and Target."
knitr::kable(pathlength.model.outputs, "latex", booktabs=T, longtable=T, caption=cap, digits=2, align="c") %>%
...
A series of four model comparisons were run using linear mixed-effects regression to test mean path length as a dependent variable, with data type and age as fixed effects and speaker as a random effect. Two models were run for each corpus, one comparing Actual data with equivalent Watts-Strogatz and Erdős–Rényi simulations, and the other comparing these simulated datasets with Target data. All four model comparisons revealed a significant main effect for Data type; see Table \@ref(tab:table-pathlength-modeloutput). Next, follow-up models were run for each corpus on only Target/Actual data (Real Target vs. Simulated Target vs. Erdős–Rényi, and Real Actual vs. Simulated Actual vs. Erdős–Rényi) to establish differences between the Real and Simulated data. Results are shown in Table \@ref(tab:table-pathlength); for both Actual and Target data in both corpora, the Real data differed significantly from the random Erdős–Rényi network, as predicted. In both cases the Actual data did not differ from the Simulated Watts-Strogatz model, though the opposite was true for the Target data. These outcomes are consistent with predictions in H3.

```{r Figure-path-length, echo=FALSE, fig.cap=cap, fig.pos='H', message=FALSE, warning=FALSE}
plot(path_length_all)
cap <- sprintf("Change in mean path length over time, in Real data (Actual or Target) compared with Watts-Strogatz and Erdős–Rényi simulations representing small-world and random network growth properties, respectively. Coloured lines represent Data type; grey bands represent 95%% CIs.")
```

```{r Figure-clust-coef, echo=FALSE, fig.cap=cap, fig.pos='H', message=FALSE, warning=FALSE}
#par(cex=1.2)
plot(clust_coef_all)
cap <- sprintf("Change in mean clustering coefficient over time, in Real data (Actual or Target) compared with Watts-Strogatz and Erdős–Rényi simulations representing small-world and random network growth properties, respectively. Coloured lines represent Data type; grey bands represent 95%% CIs.")
```

## Clustering Coefficient

Similarly, average clustering coefficient was calculated for each child's monthly networks using the *igraph()* package, for both the Real and Simulated data. As above, mixed effects linear regression models were generated for each corpus; the same model structure was used, but this time with average clustering coefficient as the dependent variable.

```{r small world properties - clustering coefficient, message=FALSE, warning=FALSE, include=FALSE}

# Providence

# Actual

global_clustcoef_model_A_P <- lmerTest::lmer(clust_coef_avg ~ data_type * age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "English" &
                                                             data_type %in% c("actual", "WS_actual", "Erdos_Renyi")
                                                           ),
                                       REML=FALSE)
#summary(global_clustcoef_model_A_P)

global_clustcoef_model_A_P_null <- lmerTest::lmer(clust_coef_avg ~ age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "English"  &
                                                             data_type %in% c("actual", "WS_actual", "Erdos_Renyi")),
                                       REML=FALSE)

clustcoef_WS_A_P <- anova(global_clustcoef_model_A_P, global_clustcoef_model_A_P_null)

# Target

global_clustcoef_model_T_P <- lmerTest::lmer(clust_coef_avg ~ data_type * age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "English" &
                                                             data_type %in% c("target", "WS_target", "Erdos_Renyi")
                                                           ),
...
global_clustcoef_model_T_P_null <- lmerTest::lmer(clust_coef_avg ~ age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "English"  &
                                                             data_type %in% c("target", "WS_target", "Erdos_Renyi")),
                                       REML=FALSE)

clustcoef_WS_T_P <- anova(global_clustcoef_model_T_P, global_clustcoef_model_T_P_null)

# Lyon

# Actual

global_clustcoef_model_A_L <- lmer(clust_coef_avg ~ data_type * age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "French" &
                                                             data_type %in% c("actual", "WS_actual", "Erdos_Renyi")),
                                       REML=FALSE)

#summary(global_clustcoef_model_A_L)

global_clustcoef_model_A_L_null <- lmer(clust_coef_avg ~ age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "French" &
                                                             data_type %in% c("actual", "WS_actual", "Erdos_Renyi")),
                                       REML=FALSE)

clustcoef_WS_A_L <- anova(global_clustcoef_model_A_L, global_clustcoef_model_A_L_null)

# Target

global_clustcoef_model_T_L <- lmer(clust_coef_avg ~ data_type * age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "French" &
                                                             data_type %in% c("target", "WS_target", "Erdos_Renyi")),
                                       REML=FALSE)
...
global_clustcoef_model_T_L_null <- lmer(clust_coef_avg ~ age + (1|Speaker),
                                             data=subset(SWD_red,
                                                           corpus == "French" &
                                                             data_type %in% c("target", "WS_target", "Erdos_Renyi")),
                                       REML=FALSE)

clustcoef_WS_T_L <- anova(global_clustcoef_model_T_L, global_clustcoef_model_T_L_null)

```

```{r Clustering coefficient model outputs, include=FALSE}

table.CC.model.output<- rbind(clustcoef_WS_A_P, clustcoef_WS_T_P, clustcoef_WS_A_L, clustcoef_WS_T_L) %>%
  rownames_to_column(var="Model") %>%
  filter(Chisq > 0) %>%
  rename(#"PA Df"="Df",
         #"PA Chisq"="Chisq",
         "p"=`Pr(>Chisq)`) %>%
  mutate(Corpus = ifelse(Model == "global_clustcoef_model_A_P", "English", " "),
         Corpus = ifelse(Model == "global_clustcoef_model_A_L", "French", Corpus),
         Model=fct_recode(Model,
                           "Actual"="global_clustcoef_model_A_P",
                            "Target"="global_clustcoef_model_T_P",
                            "Actual"="global_clustcoef_model_A_L",
                            "Target"="global_clustcoef_model_T_L"),
             p=scales::pvalue(p)) %>%
  rename(`Data type` = "Model") %>%
  select(Corpus, `Data type`, `Df`, `Chisq`, `p`)

model.summary.P_A_clustcoef <- summary(global_clustcoef_model_A_P)

clustcoef_P_A_tab <- model.summary.P_A_clustcoef$coefficients %>%
  as.data.frame %>%
  rename(
    "beta"="Estimate"
     , "SE"="Std. Error"
...
model.summary.P_T_clustcoef <- summary(global_clustcoef_model_T_P)

clustcoef_P_T_tab <- model.summary.P_T_clustcoef$coefficients %>%
  as.data.frame %>%
  rename(
    "beta"="Estimate"
     , "SE"="Std. Error"
...
model.summary.L_A_clustcoef <- summary(global_clustcoef_model_A_L)

clustcoef_L_A_tab <- model.summary.L_A_clustcoef$coefficients %>%
  as.data.frame %>%
  rename(
    "beta"="Estimate"
     , "SE"="Std. Error"
...
model.summary.L_T_clustcoef <- summary(global_clustcoef_model_T_L)

clustcoef_L_T_tab <- model.summary.L_T_clustcoef$coefficients %>%
  as.data.frame %>%
  rename(
    "beta"="Estimate"
     , "SE"="Std. Error"
...
clustcoef_english <- cbind(clustcoef_P_A_tab, clustcoef_P_T_tab)
clustcoef_french <- cbind(clustcoef_L_A_tab, clustcoef_L_T_tab)

clustcoef.model.outputs <- rbind(clustcoef_english, clustcoef_french)
rownames(clustcoef.model.outputs) <- NULL

```

Model outputs are shown in Table \@ref(tab:table-clustcoef-modeloutput). Again, across both corpora the Real (Actual and Target) data differed significantly from the random Erdős–Rényi network. However, this time only the Actual data in the English corpus showed no difference between the Real and the Simulated Watts-Strogatz data; in the French data, this was marginally significant. Again, this lends support (albeit partial in this case) to H3. See Figure \@ref(fig:Figure-clust-coef). 

```{r table-clustcoef-modeloutput, echo=F, message=FALSE, warning=FALSE, comment=F, results="asis"}
cap="Outputs from nested model comparisons testing the effect of data type (Real vs. Watts-Strogatz vs. Erdős–Rényi) on clustering coefficient. English and French data, both Actual and Target."
knitr::kable(table.CC.model.output, "latex", booktabs=T, longtable=T, caption=cap, digits=2, align="c") %>%
  kable_styling()%>%
  row_spec(9, hline_after=T)

```

```{r table-clustcoef, echo=F, message=FALSE, warning=FALSE, comment=F, results="asis"}
cap="Outputs from linear regression models testing comparisons of Real vs. Simulated data on clustering coefficient. English and French data, both Actual and Target."
knitr::kable(clustcoef.model.outputs, "latex", booktabs=T, longtable=T, caption=cap, digits=2, align="c") %>%
  add_header_above(c(" "=2, "Actual"=4, "Target"=4)) %>%
    kable_styling()%>%
  row_spec(1, extra_latex_after = "\\cline{2-10}") %>%
  row_spec(2, extra_latex_after = "\\cline{2-10}") %>%
