...
CC <- lmerTest::lmer(clust_coef_avg ~ data_type + corpus + age + (1+age|Speaker),
                                             data=subset(SWD_red,
                                                             data_type %in% c("actual", "WS_actual", "Erdos_Renyi")
                                                           ),
                                       REML=FALSE)
...
MPL.DT <- lmerTest::lmer(path_length ~ data_type + corpus + age + (1+age|Speaker),
                                             data=subset(SWD_red,
...
There was a significant effect for data type on mean path length. See Table \@ref(tab:table-model-output). Model outputs revealed that Target data had a significantly higher mean path length than Actual data (see Table \@ref(tab:table-actual-target and Figure \@ref(fig:Figure-path-length-DT). Again there was no effect for corpus, but age significantly affected mean path length, which increased as age increased, again indicating an unexpected decrease in systematicity over time. 

```{r data type - clustering coefficient, message=FALSE, warning=FALSE, include=FALSE}

CC.DT <- lmerTest::lmer(clust_coef_avg ~ data_type + corpus + age + (1+age|Speaker),
                                             data=subset(SWD_red,
                                                             data_type %in% c("actual", "target")
                                                           ),
                                       REML=FALSE)
...
The effect of data type on clustering coefficient was also significant, in line with predictions, and findings from Laing [-@laing_phonological_2023]. Mean clustering coefficient was significantly lower in Target vs. Actual data. See Figure \@ref(fig:Figure-clust-coef-DT). Mean clustering coefficient decreased as age and network size increased, again indicating decreased systematicity over time.
...
This paper set out to test the presence of systematicity in infants' developing lexicon by analysing phonological network graphs from nine infants acquiring French or English. Network graphs allow a close-up view of how similar phonological forms are to one another within the network (mean path length) and the extent to which groups of phonologically similar forms cluster together within the network (average clustering coefficient). Systematicity in the network would be reflected in shorter distance between forms and higher clusters of phonologically similar forms.

I first tested whether systematicity could be observed in the data through the analysis of network graphs, following on from previous work [@laing_phonological_2023] where I found that network growth algorithms could predict word learning based on the fact that newly-acquired words were phonologically similar to known words in the lexicon. These findings were supported in the present analysis: when compared with a randomly-generated network of equal size, the phonological networks were found to be structured in a significantly more systematic way, in terms of both mean path length and clustering coefficient.

However, when the phonological networks were compared with a prototypical 'small-world' network, which possesses a highly systematic structure by design [@watts_collective_1998], both mean path length and clustering coefficient were found to be less systematic than the prototypical model. To interrogate this further, a follow-up analysis comparing infants' Actual productions with the Target forms was conducted. This allowed an analysis of network properties based on an 'adult' model, as opposed to a prototypical model of systematicity that does not necessarily reflect real-world linguistic phenomena. Indeed, in this analysis infants' productions were found to be more systematic across both measures than we would expect from an analysis of Target forms alone, again supporting findings above, as well as initial predictions.

One surprising outcome of this analysis was the *decrease* in systematicity over time. Contrary to predictions set out above, mean path length increased over time, and clustering coefficient decreased, indicating more sparsely-distributed forms in the network. This is also in direct contrast with findings from Laing [-@laing_phonological_2023]. However, it is possible that this outcome is an artifact of the data: as network size increases, it seems plausible that we would also expect the magnitude of the network properties under investigation to increase in size. While many nodes may still be closely-connected (via low mean path length) and tightly-clustered (via high clustering coefficient), the acquisition of new forms that are likely to increase the phonological variability of the network no doubt sway these measures towards higher and lower average values, respectively. 
